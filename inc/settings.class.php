...
class TONJOO_PWA_SETTINGS {
    ...
    function callback_checkbox( $args ) {
        $value = esc_attr( $this->get_option( $args['id'], $args['section'], $args['std'] ) );
        $size = isset( $args['size'] ) && !is_null( $args['size'] ) ? $args['size'] : 'checkbox';

        $html = '<fieldset>';
        $html .= sprintf( '<label for="%1$s[%2$s]">', $args['section'], $args['id'] );
        $html .= sprintf( '<input type="hidden" name="%1$s[%2$s]" value="off" />', $args['section'], $args['id'] );
        $html .= sprintf( '<input type="checkbox" name="%1$s[%2$s]" id="%1$s[%2$s]" class="%3$s" value="on" %4$s />', $args['section'], $args['id'], $size, checked( $value, 'on', false ) );
    ...
    /**
     * Displays a file permissions field for a settings field
     *
     * @param array   $args settings field args
     */
    function callback_permissions( $args ) {
        ob_start();
